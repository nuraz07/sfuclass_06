# infra/envs/prod/media-edge.us-east-1.tfvars
#
# Media plane of PRODUCTION in us-east-1 — second media region, the Americas.
# Rooms are placed here when the host's region hint points here and the tenant allows it; the control plane stays
# in eu-central-1 and is reached over Transit Gateway peering (control traffic only, never media).
# use1-az3 is left out on purpose: newer instance families are often not offered there. Check with
#   aws ec2 describe-instance-type-offerings --location-type availability-zone-id --region us-east-1
# before changing the zone list.
#
#   tofu -chdir=infra/media-edge init  -backend-config=../envs/prod/backend.hcl \
#                                       -backend-config="key=media-edge/us-east-1.tfstate"
#   tofu -chdir=infra/media-edge apply -var-file=../envs/prod/media-edge.us-east-1.tfvars
...
vpc_cidr            = "10.65.0.0/16" # inside the prod media_edge_supernet 10.64.0.0/10, unique per region
transit_gateway_asn = 64514          # hub 64512 · eu-central-1 64513 · us-east-1 64514 · ap-southeast-1 64515
