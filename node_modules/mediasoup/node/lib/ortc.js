...
function generateRouterRtpCapabilities(mediaCodecs = []) {
    ...
    validateAndNormalizeRtpCapabilities(supportedRtpCapabilities_1.supportedRtpCapabilities);
...
function serializeRtpMapping(builder, rtpMapping) {
    ...
    for (const encoding of rtpMapping.encodings) {
        encodings.push(FbsRtpParameters.EncodingMapping.createEncodingMapping(builder, builder.createString(encoding.rid), encoding.ssrc ?? null, encoding.mappedSsrc));
    }
 ...
 */
function validateAndNormalizeRtpHeaderExtensionParameters(ext) {
    if (typeof ext !== 'object') {
        throw new TypeError('ext is not an object');
    }
    // uri is mandatory.
