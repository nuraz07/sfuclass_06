...
const FbsConsumer = __importStar(require("./fbs/consumer"));
const FbsDataConsumer = __importStar(require("./fbs/data-consumer"));
const FbsDataProducer = __importStar(require("./fbs/data-producer"));
const FbsTransport = __importStar(require("./fbs/transport"));
const FbsRouter = __importStar(require("./fbs/router"));
const FbsRtpParameters = __importStar(require("./fbs/rtp-parameters"));
...
class TransportImpl extends enhancedEvents_1.EnhancedEventEmitter {
    ...
    #sctpNegotiatedMaxOutboundStreams;
    // Next SCTP stream id.
    #nextSctpStreamId = 0;
    // Observer instance.
    #observer;
    ...
    async setMaxIncomingBitrate(bitrate) {
        ...
        /* Build Request. */
        const requestOffset = FbsTransport.SetMaxIncomingBitrateRequest.createSetMaxIncomingBitrateRequest(this.channel.bufferBuilder, BigInt(bitrate));
    ...
    async setMaxOutgoingBitrate(bitrate) {
        ...
        /* Build Request. */
        const requestOffset = new FbsTransport.SetMaxOutgoingBitrateRequestT(BigInt(bitrate)).pack(this.channel.bufferBuilder);
    ...
    async setMinOutgoingBitrate(bitrate) {
        ...
        /* Build Request. */
        const requestOffset = new FbsTransport.SetMinOutgoingBitrateRequestT(BigInt(bitrate)).pack(this.channel.bufferBuilder);
    ...
    async consume({ producerId, rtpCapabilities, paused = false, mid, preferredLayers, ignoreDtx = false, enableRtx, pipe = false, appData, }) {
        ...
        if (!producer) {
            throw new errors_1.NotFoundError(`Producer with id "${producerId}" not found`);
        }
    ...
    async consumeData({ dataProducerId, ordered, maxPacketLifeTime, maxRetransmits, paused = false, subchannels, appData, }) {
        ...
        if (!dataProducer) {
            throw new errors_1.NotFoundError(`DataProducer with id "${dataProducerId}" not found`);
        }
        let type;
    ...
    allocateSctpStreamId() {
        ...
        if (this.#sctpNegotiatedMaxOutboundStreams !== undefined &&
            this.#nextSctpStreamId + 1 > this.#sctpNegotiatedMaxOutboundStreams) {
            logger.warn(`SCTP negotiated max outbound streams is higher than current next SCTP streamId + 1, resetting it to 0`);
...
function parseBaseTransportDump(binary) {
    ...
    if (fbsSctpParameters) {
        sctpParameters = (0, sctpParametersFbsUtils_1.parseSctpParameters)(fbsSctpParameters);
    }
    // Retrieve sctpState.
...
function parseBaseTransportStats(binary) {
    ...
    return {
        ...
        availableOutgoingBitrate: binary.availableOutgoingBitrate() !== null
            ? Number(binary.availableOutgoingBitrate())
            : undefined,
        availableIncomingBitrate: binary.availableIncomingBitrate() !== null
            ? Number(binary.availableIncomingBitrate())
            : undefined,
        maxIncomingBitrate: binary.maxIncomingBitrate() !== null
            ? Number(binary.maxIncomingBitrate())
            : undefined,
        maxOutgoingBitrate: binary.maxOutgoingBitrate() !== null
            ? Number(binary.maxOutgoingBitrate())
            : undefined,
        minOutgoingBitrate: binary.minOutgoingBitrate() !== null
...
function parseSctpNegotiatedCapabilitiesDump(binary) {
    return {
